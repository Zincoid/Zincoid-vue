...
const parsed = computed(() => {
  ...
  const platforms = {
    website:  { icon: 'website', label: t('common.website'), hover: 'green',  href: v => v.startsWith('http') ? v : `https://${v}` },
    email:    { icon: 'email',   label: t('common.email'),   hover: 'yellow', href: v => `mailto:${v}` },
    github:   { icon: 'github',  label: 'GitHub',            hover: 'blue',   href: v => v },
    bilibili: { icon: 'bilibili',label: 'Bilibili',          hover: 'pink',   href: v => v },
    x:        { icon: 'x',       label: 'X',                 hover: 'black',  href: v => v }
  }

  const order = ['email', 'website', 'github', 'bilibili', 'x']

  return Object.entries(obj)
    .filter(([, v]) => v)
    .map(([key, value]) => {
      const p = platforms[key] || { icon: 'link', label: key, hover: null, href: v => v.startsWith('http') ? v : `https://${v}` }
...
<template>
  <div class="contact-buttons" :class="{ 'contact-buttons--icon-only': iconOnly }" v-if="parsed.length">
    <a
      ...
      :href="c.href(c.value)"
      class="contact-btn"
      :class="[`contact-btn--${c.icon}`, c.hover && `contact-btn--hover-${c.hover}`]"
      target="_blank"
...
.contact-btn--hover-green:hover  { border-color: #16a34a; color: #16a34a; background: #dcfce7; }
.contact-btn--hover-yellow:hover { border-color: #d97706; color: #d97706; background: #fef3c7; }
.contact-btn--hover-blue:hover   { border-color: #2563eb; color: #2563eb; background: #dbeafe; }
.contact-btn--hover-pink:hover   { border-color: #db2777; color: #db2777; background: #fce7f3; }
.contact-btn--hover-black:hover  { border-color: #111827; color: #111827; background: #e5e7eb; }
